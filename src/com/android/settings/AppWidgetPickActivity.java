...
public class AppWidgetPickActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.hasExtra(AppWidgetManager.EXTRA_APPWIDGET_ID)) {
			fAppWidgetId = intent.getIntExtra(
	...
	public void finishOk(SubItem item) {
		...
		if (item.getExtra() != null) {
			...
			try {
				...
				// it's not clear that that's useful -- the widget will simply
				// not
				// appear.
	...
	/**
	 * Build the {@link Intent} described by this item. If this item can't
	 * create a valid {@link ComponentName}, it will return
	...
	private Intent getIntent(SubItem itm) {
		...
		if (itm.getProvider() != null) {
			...
			intent.setClassName(itm.getProvider().getPackageName(), itm
					.getProvider().getClassName());
		} else {
	...
	private Item getPackageItem(AppWidgetProviderInfo info) {
		...
		try {
			android.content.pm.ApplicationInfo appInfo = fPManager
					.getApplicationInfo(info.provider.getPackageName(), 0);
	...
	private void AddAppWidgetProviderInfos() {
		List<AppWidgetProviderInfo> infos = fAppWManager
		...
		for (AppWidgetProviderInfo info : infos) {
			try {
				android.content.pm.ApplicationInfo appInfo = fPManager
						.getApplicationInfo(info.provider.getPackageName(), 0);
				SubItem itm = new SubItem(info.label, fPManager.getDrawable(
						info.provider.getPackageName(), info.icon, appInfo));
	...
	private void AddCustomAppWidgets() {
		...
		try_custom_items: {
			customInfo = extras
					.getParcelableArrayList(AppWidgetManager.EXTRA_CUSTOM_INFO);
			...
			customExtras = extras
					.getParcelableArrayList(AppWidgetManager.EXTRA_CUSTOM_EXTRAS);
	...
	private void putAppWidgetItems(List<AppWidgetProviderInfo> appWidgets,
			List<Bundle> customExtras) {
		if (appWidgets == null)
			return;
		...
		for (int i = 0; i < size; i++) {
			...
			if (info.icon != 0) {
				icon = fPManager.getDrawable(info.provider.getPackageName(),
						info.icon, null);
			}
